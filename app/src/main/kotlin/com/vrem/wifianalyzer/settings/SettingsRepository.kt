    ...
    constructor(
        ...
        fun toSettingsData(preferences: Preferences): SettingsData {
            ...
            val wiFiBand =
                findOne(
                    WiFiBand.entries,
                    preferences[wiFiBandKey]?.toIntOrNull() ?: WiFiBand.GHZ2.ordinal,
                    WiFiBand.GHZ2,
                )
            ...
            val sortBy =
                findOne(
                    SortBy.entries,
                    preferences[sortByKey]?.toIntOrNull() ?: SortBy.STRENGTH.ordinal,
                    SortBy.STRENGTH,
                )
            val groupBy =
                findOne(GroupBy.entries, preferences[groupByKey]?.toIntOrNull() ?: GroupBy.NONE.ordinal, GroupBy.NONE)
            val accessPointView =
                findOne(
                    AccessPointViewType.entries,
                    preferences[apViewKey]?.toIntOrNull() ?: AccessPointViewType.COMPLETE.ordinal,
                    AccessPointViewType.COMPLETE,
                )
            val connectionViewType =
                findOne(
                    ConnectionViewType.entries,
                    preferences[connectionViewKey]?.toIntOrNull() ?: ConnectionViewType.COMPACT.ordinal,
            ...
            val selectedMenu =
                findOne(
                    ...
                    preferences[selectedMenuKey]?.toIntOrNull() ?: NavigationMenu.ACCESS_POINTS.ordinal,
